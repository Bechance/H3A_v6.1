...
void Updata_str_view( char*notice )
{
	...
	GUI_string_display_At( 5, m_updaBAR->win_high + 25, "正在升级 " );
...
void app_Extapp_updata( void )
{
	...
	{
		key_buf = MsgBox( 200, 200, 250, 120, ( char* )"提示", ( char* )"没有搜索到U盘.", APP_HALF_SEC * 100 );
	...
	GUI_set_Fgcolor( C_GUI_RED );
	GUI_SetFront( &GUI_HZK16_EXTASCII );
	GUI_string_display_At( 5, m_updaBAR->win_high + 25, "升级结束，按任意键退出.     " );
	GUI_window_hwin_release( m_updaWIN );
