...
uint8_t 		m_WIFImod_Cline_task_pro = INVALID_PRO;
WIR_ALG_t*	    m_alg_presult;
a_sem_t *       m_WIFImod_sem            = INVALID_SEM;
a_sem_t *       m_WIFImod_Resem          = INVALID_SEM;
 ...
 *@输出  :NONE
********************************************************************/
void app_WIFImod_DL_param_obtain(measure_param_t*pDL)
{
	Ua = pDL->phase_a.f_U;
	Ia = pDL->phase_a.f_I;
	Pa = pDL->phase_a.f_P;
 ...
 *@输出  :NONE
********************************************************************/
void  app_WIFImod_check_err_task( void*arg )
{
	...
	while ( 1 )
	{
        ...
        if(!dl_flag.DL_data_is_new)continue;
		app_global_DLarg_obtain(&g_DL_param);
		if ( !_angle_sys( &g_DL_param ) )continue;
		app_WIFImod_DL_param_obtain(&g_DL_param);
...
void app_WIFImod_alg_entery( void )
{
	...
	os_sem_creat( &m_WIFImod_sem, 0 );
	os_sem_creat( &m_WIFImod_Resem, 0 );
	m_alg_presult = app_malloc( sizeof( WIR_ALG_t ) * 4 );/*存放3组*/
	m_WIFImod_Cline_task_pro
	= os_task_assign_stk_build( app_WIFImod_check_err_task, 4 );
	os_task_name_set( m_WIFImod_Cline_task_pro, "WIFImod--ALG task" );
	_angle_FIFO[1] = 1;
	_angle_FIFO[0] = 1;
	_timecnt   = 95;
	return;
...
{
	os_task_delete( m_WIFImod_Cline_task_pro );
