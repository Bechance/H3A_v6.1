...
hwin_t*     m_zz_XNFZ_win = 0;
hwin_t*     m_zz_XNFZ_edit = 0;
Bsize_t  	m_sec;
uint8_t   task_XNFZ_id 	= INVALID_PRO;

extern  dev_id_t        g_UIout_dev;
#define XNFZ_WIN        m_zz_XNFZ_edit
...
static void _ARG_load(void)
{
	...
	app_zzmod_measure_auto_change_Ilevel(UIoutOP.f_I[A_PHASE]);
...
void app_zz_xnfz_window_create( void )
{
	...
	m_sec = GUI_read_block_Extp(x1-5,y1,x2+10, y2+5);
	GUI_window_hwin_req(GET_GUIWIN);
	Gui_button_set_color(C_GUI_LOW_BLUE,zz_WIN_BG_COLOR);
	Gui_3D_window_set_color(C_GUI_WHITE,zz_WIN_BG_COLOR,C_GUI_RED);
	GUI_set_Bgcolor( zz_WIN_BG_COLOR );
	m_zz_XNFZ_win = Gui_3D_window_creat(x1, y1, x2-x1,y2-y1,"虚拟负载",WIN_MSG_EN|WIN_3D|WIN_FILL, 0 );
	GUI_window_hwin_req(m_zz_XNFZ_win);
    m_zz_XNFZ_edit = GUI_edit_creat(0,0,314,330,WIN_MSG_EN|WIN_3D|WIN_TOUCH_EN,m_zz_XNFZ_win);
	_XNFZ_init();
	GUI_SetFront(&GUI_HZK16_EXTASCII);
	pOBJ=Gui_button_creat(60,275,70,25,BUTTON_PAINT|GUI_KEY_XNFZ_START,m_zz_XNFZ_edit);
	GUI_button_title_display(pOBJ,(char *)m_start[g_language]);
	pOBJ=Gui_button_creat(190,275,70,25,BUTTON_PAINT|GUI_KEY_XNFZ_CLOSE,m_zz_XNFZ_edit);
	GUI_button_title_display(pOBJ,(char *)m_stop[g_language]);
	GUI_TopWIN_Set(m_zz_XNFZ_edit);
	GUI_window_hwin_release(m_zz_XNFZ_win);
...
void app_zz_xnfz_window_destory( void )
{
	...
	GUI_window_destory(m_zz_XNFZ_edit);
	GUI_window_destory(m_zz_XNFZ_win);
	GUI_write_block_Extp(x1-5,y1,x2+10, y2+5, m_sec);
...
void app_zz_XNFZ_task(void*arg)
{
	...
	{   
		...
		{
			app_zz_xnfz_window_destory();
			os_task_delete( task_XNFZ_id );
			task_XNFZ_id = INVALID_PRO;
		}
