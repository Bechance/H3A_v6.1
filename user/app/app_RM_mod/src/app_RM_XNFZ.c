...
hwin_t*     m_RM_XNFZ_win 	= 0;
hwin_t*     m_RM_XNFZ_edit 	= 0;
Bsize_t  	m_RM_sec;
uint8_t   	task_RM_XNFZ_id = INVALID_PRO;

extern  dev_id_t        g_UIout_dev;
#define XNFZ_WIN        m_RM_XNFZ_edit
...
static void _ARG_load(void)
{
	...
	app_RMmod_measure_auto_change_Ilevel(UIoutOP.f_I[A_PHASE]);
...
void app_RMmod_xnfz_window_create( void )
{
	...
	m_RM_sec = GUI_read_block_Extp(x1-5,y1,x2+10, y2+5);
	GUI_window_hwin_req(GET_GUIWIN);
	Gui_button_set_color(C_GUI_LOW_BLUE,RM_WIN_BG_COLOR);
	Gui_3D_window_set_color(C_GUI_WHITE,RM_WIN_BG_COLOR,C_GUI_RED);
	GUI_set_Bgcolor( RM_WIN_BG_COLOR );
	m_RM_XNFZ_win = Gui_3D_window_creat(x1, y1, x2-x1,y2-y1,"虚拟负载",WIN_MSG_EN|WIN_3D|WIN_FILL, 0 );
	GUI_window_hwin_req(m_RM_XNFZ_win);
    m_RM_XNFZ_edit = GUI_edit_creat(0,0,314,330,WIN_MSG_EN|WIN_3D|WIN_TOUCH_EN,m_RM_XNFZ_win);
	_XNFZ_init();
	GUI_SetFront(&GUI_HZK16_EXTASCII);
	pOBJ=Gui_button_creat(60,275,70,25,BUTTON_PAINT|GUI_KEY_XNFZ_START,m_RM_XNFZ_edit);
	GUI_button_title_display(pOBJ,(char *)m_start[g_language]);
	pOBJ=Gui_button_creat(190,275,70,25,BUTTON_PAINT|GUI_KEY_XNFZ_CLOSE,m_RM_XNFZ_edit);
	GUI_button_title_display(pOBJ,(char *)m_stop[g_language]);
	GUI_TopWIN_Set(m_RM_XNFZ_edit);
	GUI_window_hwin_release(m_RM_XNFZ_win);
...
void app_RMmod_xnfz_window_destory( void )
{
	...
	GUI_window_destory(m_RM_XNFZ_edit);
	GUI_window_destory(m_RM_XNFZ_win);
	GUI_write_block_Extp(x1-5,y1,x2+10, y2+5, m_RM_sec);
...
//三相源开关
void app_RM_XNFZ_UIout_SW(const void*str)
{
...
//三相源通道设置
void app_RM_XNFZ_UIout_EN(BOOL U_EN,BOOL I_EN)
{
...
void app_RM_XNFZ_task(void*arg)
{
	...
	{   
		...
		{
			app_RMmod_xnfz_window_destory();
			os_task_delete( task_RM_XNFZ_id );
			task_RM_XNFZ_id = INVALID_PRO;
		}
