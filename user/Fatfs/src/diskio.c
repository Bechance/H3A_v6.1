...
{
	os_driver_ioctl( g_spi_dev_id, SET_SD_CS_EN, 0 );
	return;
}
void SD_cs_disable( void )
{
	os_driver_ioctl( g_spi_dev_id, SET_SD_CS_DIS, 0 );
	return;
}

void SD_set_baud( unsigned long baud )
{
	long  lspclk;
	lspclk = 30000000;
	SpiaRegs.SPIBRR = ( lspclk ) / baud - 1;
	return;
}
...
static  BYTE xchg_spi( BYTE Write_BUF )
{
	SpiaRegs.SPITXBUF = ( Write_BUF << 8 );
	SpiaRegs.SPISTS.bit.BUFFULL_FLAG = 1;
	while ( SpiaRegs.SPISTS.bit.INT_FLAG == 0 );
	return ( unsigned char )SpiaRegs.SPIRXBUF;
}
...
/* platform MACRO*/
/*-----------------------------------------------------------------------*/
#define CS_LOW()	 SD_cs_enable()			   	   	/* CS=low */
#define	CS_HIGH()	 SD_cs_disable()			   	/* CS=high */
...
#define	FCLK_SLOW()	 {SpiaRegs.SPIBRR = 0x63;}	    /* Set slow clock (F_CPU / 64) */
#define	FCLK_FAST()	 {SpiaRegs.SPIBRR = 0X08;}		/* Set fast clock (F_CPU / 2) */
/*--------------------------------------------------------------------------
...
static
void xmit_spi_multi(
	const BYTE *buff,	/* Data block to be sent */
	UINT btx		/* Size of data block */
)
{
	while ( btx-- )
	{
