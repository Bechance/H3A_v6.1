...
/                   Moved file lock semaphore table from m_fatfs object to the bss.
...
#if _VOLUMES
FATFS *FatFs[_VOLUMES] = {0};	/* Pointer to the file system objects (logical drives) */
#else
...
#define INIT_BUF(dobj)		{ lfn = ff_memalloc((_MAX_LFN + 1) * 2); \
		if (!lfn) LEAVE_FF((dobj).m_fatfs, FR_NOT_ENOUGH_CORE); \
